...
from app.providers.google.gmail.api import stage_email, read_emails
from app.providers.google.calendar.api import get_calendar_events, create_calendar_event
...
Document rules:
- Whenever the user references a document, contract, report, pitch deck,
  invoice, or any uploaded file, ALWAYS call search_my_documents first
  before composing any response or email.
- NEVER fabricate or paraphrase document content from memory — only quote
  from text returned by search_my_documents or get_document_summary.
- If search_my_documents returns no relevant content, tell the user
